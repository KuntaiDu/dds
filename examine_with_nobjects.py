...
def eval(max_fid, map_dd, map_gt, gt_confid_thresh, mpeg_confid_thresh, max_area_thresh_gt, max_area_thresh_mpeg):
	...
	fn = np.array(fn_list)
	count = count_list
	return tp, fp, fn, count, tp/(tp+fp), tp/(tp+fn), (2.0*tp/(2.0*tp+fp+fn)).tolist()
...
for file in dirs:
	...
	if "req_regions" in file or "jpg" in file or "segment_size" in file or '0.3' in file or os.path.isdir(os.path.join(results_direc,file)): continue
