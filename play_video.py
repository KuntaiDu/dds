...
def main(args):
    ...
    if not args.baseline:
        if args.simulate:
            ...
            # Run simulation
            logger.info(f"Analyzing video {args.video_name} "
                        f"with low resolution "
            ...
            results, bw = client.analyze_video_simulate(args.video_name,
                                                        ...
                                                        args.debug_mode)
        elif not args.resolutions[-1] == -1:
            mode = "emulation"
            logger.warning(f"Running DDS in EMULATION mode on "
                           f"{args.video_name}")
            # Run emulation
        ...
        if args.baseline == "vigil":
            logger.warning(f"Running in Vigil mode")
            results, bw = client.analyze_video_vigil(
                args.video_name, args.high_images_path)
        elif args.baseline == "glimpse":
            logger.warning(f"Running in Glimpse mode")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--baseline", dest="baseline", type=str, default=None,
                        help="If given, will run for the given baseline")
