...
def simple_merge(single_result_frame, index_to_merge):
    ...
    for i in index_to_merge:
        ...
        new_origin = "low-res"
        # If both low
        if "low" in left.origin and "low" in right.origin:
            # If one of them was recorded in the second phase
            if "second-phase" in left.origin or "second-phase" in right.origin:
                new_origin = "low-res-second-phase"
        # If any one of them is in high
        if "high" in left.origin or "high" in right.origin:
            new_origin = "high-res"
