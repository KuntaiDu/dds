...
def eval(max_fid, map_dd, map_gt, gt_confid_thresh, mpeg_confid_thresh, max_area_thresh_gt, max_area_thresh_mpeg):
	...
	fn_list = []
	count_list = []
	for fid in [i for i in range(max_fid+1) if i in map_dd.keys()]:
...
def obtain_file_names(stats_file):
    names = []
    with open(stats_file, 'r') as f:
        for line in f.readlines():
            split = line.split(',')
            if split[0] == 'video-name':
                continue
...
for file in dirs:
	...
	if "req_regions" in file or "segment_size" in file or "jpg" in file or os.path.isdir(os.path.join(results_direc,file)):
		continue
