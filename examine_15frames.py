...
relevant_classes = env_2_relevant_classes[0]
...
def eval(max_fid, map_dd, map_gt, gt_confid_thresh, mpeg_confid_thresh, max_area_thresh_gt, max_area_thresh_mpeg):
	...
	return tp, fp, fn, count, myround(tp, tp+fp), myround(tp, tp+fn), myround(2.0*tp , 2.0*tp+fp+fn)
...
if gt_mode == 'gt':
	gt_key = video_name + "_mpeg_1.0_24"
