...
DATASET_ORIGIN=/data/yuanx/new_dataset
OUTPUT_FILE=stats
...
then
    ...
    low=$6
    high=$7
    low_results=backend/no_filter_denoise_combined_merged_bboxes/${video}_mpeg_${low}_${low_qp}
    python play_video.py --vid-name results/${vname} --high-src ${original_images} \
           ...
           --ground-truth results/${video}_mpeg_0.9_30 --max-size 0.3 \
           --low-threshold 0.3 --high-threshold 0.3 --enforce-iframes --qp ${low_qp} ${high_qp} --verbosity info
 fi
