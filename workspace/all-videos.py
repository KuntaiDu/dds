...
for video_target in vid_names:
    ...
    if CODE==0:
        ...
        vname = f'{video}_gt'
        myrun(f"python ./workspace/run_single.py gt {video} {vname} {qp} {res}")
        # Generate mpeg
        for res, qp in mpeg_configs[video]:
            print(str(res) + '/' + str(qp))
            vname = f"{video}_mpeg_{res}_{qp}"
            if True:
                myrun(f"python ./workspace/run_single.py mpeg {video} {vname} {qp} {res}")
            else:
    ...
    if CODE==1:
        ...
        for low_res, low_qp, high_res, high_qp, rpn_enlarge_ratio, batch_size, rpn_box_source, prune_score, objfilter_iou, size_obj in dds_config[video]:
            ...
            if True:
            # create dataset
                print(f"create {video}_{low_res}_{low_qp} and {video}_{high_res}_{high_qp} for RPN")
                scale=f"{1280*low_res}:{int(720*low_res)}"
                myrun(f"python ./experiment-scripts-xin/prepare_data_for_RPN.py {video} {low_res} {low_qp} {num_frames} {scale}")
                myrun(f"python ./experiment-scripts-xin/prepare_data_for_RPN.py {video} {high_res} {high_qp} {num_frames} {scale}")
            # run RPN
    ...
    if CODE==2:
        for low_res, low_qp, high_res, high_qp, rpn_enlarge_ratio, batch_size, rpn_box_source, prune_score, objfilter_iou, size_obj in dds_config[video]:
            if batch_size:
                ...
                myrun(f"python ./workspace/run_single.py dds {video} {vname} {low_qp} {high_qp} {low_res} {high_res} {rpn_enlarge_ratio} {rpn_box_source} {batch_size} {prune_score} {objfilter_iou} {size_obj}")
            else:
        ...
        # if not os.path.exists(os.path.join(result_target, vname)):
                myrun(f"python ./workspace/run_single.py dds {video} {vname} {low_qp} {high_qp} {low_res} {high_res} {rpn_enlarge_ratio} {rpn_box_source} {15} {prune_score} {objfilter_iou} {size_obj}")
        # else:
            # print(f"Skipping {vname}")
